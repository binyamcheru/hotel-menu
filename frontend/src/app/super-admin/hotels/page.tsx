                    ...
                    <div className="overflow-x-auto">
                        <table className="w-full text-left border-collapse">
                            ...
                            <tbody className="divide-y divide-gray-50">
                                {hotels.map((hotel) => (
                                    <tr key={hotel.hotel_id} className="hover:bg-gray-50 transition-colors group">
                                        <td className="px-6 py-6 font-bold text-gray-900">{hotel.name}</td>
                                        <td className="px-6 py-6 text-gray-500 font-medium">{hotel.hotel_id.substring(0, 8)}</td>
                                        <td className="px-6 py-6 text-gray-500">Manager</td>
                                        <td className="px-6 py-6">
                                            <span className="px-3 py-1 rounded-full text-[10px] font-black uppercase bg-green-50 text-green-600">
                                                ACTIVE
                                            </span>
                                        </td>
                                        <td className="px-6 py-6 text-gray-500">{new Date(hotel.created_at).toLocaleDateString()}</td>
                                        <td className="px-6 py-6 text-right">
                                            <div className="flex justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                                ...
                                                <Link href={`/super-admin/hotels/edit/${hotel.hotel_id}`} className="p-2 hover:bg-white rounded-xl shadow-sm border border-transparent hover:border-gray-100 text-gray-400 hover:text-indigo-600 transition-all">
                                                    <Edit className="w-4 h-4" />
                                                </Link>
                                                <button
                                                    onClick={() => handleDelete(hotel.hotel_id, hotel.name)}
                                                    className="p-2 hover:bg-white rounded-xl shadow-sm border border-transparent hover:border-gray-100 text-gray-400 hover:text-red-600 transition-all"
                                                >
