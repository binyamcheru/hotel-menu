...
func (r *menuItemRepository) GetDetailByID(ctx context.Context, id uuid.UUID) (*domain.MenuItemDetail, error) {
	...
	avgQuery := `SELECT COALESCE(AVG(rating), 0), COUNT(*) FROM ratings WHERE menu_item_id = $1`
	...
	feedbackRows, err := r.db.Query(ctx, feedbackQuery, id)
	if err == nil {
		defer feedbackRows.Close()
		for feedbackRows.Next() {
			var fb domain.Feedback
			if err := feedbackRows.Scan(&fb.FeedbackID, &fb.HotelID, &fb.MenuItemID,
				&fb.Message, &fb.Fingerprint, &fb.CreatedAt); err == nil {
				detail.Feedbacks = append(detail.Feedbacks, fb)
			}
		}
	}
	if detail.Feedbacks == nil {
		detail.Feedbacks = []domain.Feedback{}
	}
